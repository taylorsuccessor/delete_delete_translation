...
@section('content')
    ...
    {!! Form::model($request,['url' => route('admin.role.index'), 'class' => 'form-horizontal']) !!}
...
@if($request->old() !== null && is_array($request->old('permissionOneText')))
...
@endif
                ...
                <div class="addPermissionForm  col-sm-12">
                    ...
                    <div class="form-group {{ isset($errors) && $errors->has('permissionOneText') ? 'has-error' : ''}}  col-xs-2">
                        <div class="col-sm-12">
                            {!! Form::select('addSection_'.$sectionIndex,['*'=>trans('general.all')]+$aSection, null, ['class' => 'form-control']) !!}
                            {!! isset($errors) ? $errors->first('permissionOneText', '<p class="help-block">:message</p>'):'' !!}
                        </div>
                    </div>
@endforeach
                    <div class="  col-xs-4">
            ...
            <div class="addDenyPermissionAllForm">
                <div class="newDenyPermission">
                    ...
                    @if($request->old() != null && is_array($request->old('denyPermissionOneText')))
                ...
                <div class="addDenyPermissionForm  col-sm-12">
                    @foreach(config('permission.permissionsSections') as $sectionIndex=>$aSection)
                        <div class="form-group {{ isset($errors) && $errors->has('denyPermissionOneText') ? 'has-error' : ''}}  col-xs-2">
                            <div class="col-sm-12">
                                {!! Form::select('addSection_'.$sectionIndex,['*'=>trans('general.all')]+$aSection, null, ['class' => 'form-control']) !!}
                                {!! isset($errors)? $errors->first('denyPermissionOneText', '<p class="help-block">:message</p>'):'' !!}
                            </div>
                        </div>
                    @endforeach
                    <div class="  col-xs-4">
