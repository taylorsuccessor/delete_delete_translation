<?php namespace App\module\upload_file\admin\repository;

use Session;
use App\module\upload_file\model\UploadFile;
use App\module\upload_file\admin\Repository\UploadFileContract;
...
class EloquentUploadFileRepository implements UploadFileContract
{
    ...
    public function getByFilter($data,&$statistic=null)
    {
        ...
        if(canAccess('admin.upload_file.allData')) {
            ...
            $oResults = $oResults->where('upload_file.id', '=' , $data['id']);
        }
                if (isset($data['user_id']) && !empty($data['user_id'])) {
            $oResults = $oResults->where('upload_file.user_id', '=' , $data['user_id']);
        }
                if (isset($data['upload_group_id']) && !empty($data['upload_group_id'])) {
            $oResults = $oResults->where('upload_file.upload_group_id', '=' , $data['upload_group_id']);
        }
                if (isset($data['name']) && !empty($data['name'])) {
            $oResults = $oResults->where('upload_file.name', '=' , $data['name']);
        }
                if (isset($data['original_name']) && !empty($data['original_name'])) {
            $oResults = $oResults->where('upload_file.original_name', '=' , $data['original_name']);
        }
                if (isset($data['new_name']) && !empty($data['new_name'])) {
            $oResults = $oResults->where('upload_file.new_name', '=' , $data['new_name']);
        }
    ...
    public function create($data)
    {
        ...
        if ($result) {
            Session::flash('flash_message', 'upload_file added!');
    ...
    public function destroy($id)
    {
        ...
        if ($result) {
            Session::flash('flash_message', 'upload_file deleted!');
    ...
    public function update($id,$data)
    {
        ...
        if ($result) {
            Session::flash('flash_message', 'upload_file updated!');
