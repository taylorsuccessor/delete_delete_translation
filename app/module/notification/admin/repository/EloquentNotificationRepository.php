...
class EloquentNotificationRepository implements NotificationContract
{
    ...
    public function getByFilter($data,&$statistic=null)
    {
        ...
        if (isset($data['name']) && !empty($data['name'])) {
            $oResults = $oResults->where('notification.name', 'like' , '%'.$data['name'].'%');
        }
        if (isset($data['title']) && !empty($data['title'])) {
            $oResults = $oResults->where('notification.title', 'like' , '%'.$data['title'].'%');
        }
        ...
        if (isset($data['to_field']) && !empty($data['to_field'])) {
            $oResults = $oResults->where('notification.to_field', 'like' , '%'.$data['to_field'].'%');
        }
        if (isset($data['to_email']) && !empty($data['to_email'])) {
            $oResults = $oResults->where('notification.to_email', 'like' , '%'.$data['to_email'].'%');
        }
        ...
        if (isset($data['body']) && !empty($data['body'])) {
            $oResults = $oResults->where('notification.body', 'like' , '%'.$data['body'].'%');
        }
