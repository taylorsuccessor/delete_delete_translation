<?php
namespace App\module\system_localization\job;
...
class CheckNew implements ShouldQueue
{
    ...
    {
        $resultCollection=SystemLocalization ::where('created_at','>=',now()->subDay(4))->get();

        if(count($resultCollection)){
            notification('system_localization_check_new',[
